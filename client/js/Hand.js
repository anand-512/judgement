...
function Hand() {
    ...
    this.addCard = function (card){
        ...
        } else if(card.suit.str == Suit(d).str) {
            this.self.diamonds.push(card);
        } else if(card.suit.str == Suit(s).str) {
            this.self.spades.push(card);
        } else if(card.suit.str == Suit(h).str) {
    ...
    this.hasCardofSuit = function (suit) {
        console.log(suit);
        if(suit == "c") {
            return (this.self.clubs.length == 0) ? false: true;
        } else if(suit == "d") {
            return (this.self.diamonds.length == 0) ? false: true;
        } else if(suit == "s") {
            return (this.self.spades.length == 0) ? false: true;
        } else if(suit == "h") {
            return (this.self.hearts.length == 0) ? false: true;
        }
    };
